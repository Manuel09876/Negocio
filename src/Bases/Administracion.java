...
public class Administracion extends javax.swing.JFrame {
    ...
    public Administracion() {
        initComponents();
        addCheckBox(2, tbAdministracion);
        addCheckBox(3, tbAdministracion);
        addCheckBox(4, tbAdministracion);
        addCheckBox(5, tbAdministracion);
        addCheckBox(6, tbAdministracion);
        
        tbAdministracion.getModel().addTableModelListener(new TableModelListener() {
            @Override
            public void tableChanged(TableModelEvent e) {
                if (e.getType() == TableModelEvent.UPDATE) {
                    ...
                    if (column >= 2 && column <= 5) {
                        ...
                        for (int i = 2; i <= 5; i++) {
                            Boolean value = (Boolean) tbAdministracion.getValueAt(row, i);
                            if (value == null || !value) {
                                allSelected = false;
                                break;
                            }
                        }
                        tbAdministracion.getModel().removeTableModelListener(this);
                        tbAdministracion.setValueAt(allSelected, row, 6);
                        tbAdministracion.getModel().addTableModelListener(this);
                    } else if (column == 6) {
                        Boolean isSelected = (Boolean) tbAdministracion.getValueAt(row, column);
                        if (isSelected != null) {
                            tbAdministracion.getModel().removeTableModelListener(this);
                            for (int i = 2; i <= 5; i++) {
                                tbAdministracion.setValueAt(isSelected, row, i);
                            }
                            tbAdministracion.getModel().addTableModelListener(this);
                        }
                    }
    ...
    private void initComponents() {
        ...
        if (tbAdministracion.getColumnModel().getColumnCount() > 0) {
            tbAdministracion.getColumnModel().getColumn(0).setPreferredWidth(20);
            tbAdministracion.getColumnModel().getColumn(1).setPreferredWidth(150);
        }
